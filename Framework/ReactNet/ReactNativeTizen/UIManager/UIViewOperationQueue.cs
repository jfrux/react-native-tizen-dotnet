    ...
    public class UIViewOperationQueue
    {
            ...
            int[] tagsToDelete)
        {
            EnqueueOperation(() => {_nativeViewHierarchyManager.ManageChildren(
                ...
                indexesToRemove,
                viewsToAdd,
                tagsToDelete);/*Log.Info(ReactConstants.Tag, $"ThreadID={Thread.CurrentThread.ManagedThreadId}");*/});
        }
        ...
        {
            // TODO: Occurs when the core Silverlight rendering process renders a frame.
        ...
        private void FlushPendingBatches()
        {
            lock (_gate)
            {
                ...
                {
                    RNTracer.Error(ReactConstants.Tag, "[RN_EXCEPTION]At batch:[" + ex.ToString() + "]");
                }
                finally
                {
                    _batches.Clear();
        ...
        //EFL not support callback when each frame
        //TODO: Maybe ecore_idle can supported?
        /*
